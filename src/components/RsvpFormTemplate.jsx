...
import { Calendar, User, ArrowRight, ShieldCheck, CheckCircle2, MessageCircleQuestion, Globe, Linkedin } from 'lucide-react';
import { detectLanguage, getTranslations, languageNames } from '../app/rsvp/translations';
import Navbar from './Navbar';

/**
 * Reusable RSVP Form Template for CyberX Events
 */
export default function RsvpFormTemplate({
  eventConfig = {
    title: 'How Investigators Find Anyone Online using OSINT',
    badge: 'Live Online Event',
    speakerName: 'Saad Sarraj (cybersudo)',
    speakerTitle: 'Cyber Security Engineer',
    speakerRole: 'Speaker',
    speakerLinkedin: 'https://www.linkedin.com/in/saadsarraj/',
    eventDate: '25th July, 2026',
    eventTime: '3:00 PM IST (11:30 AM CET)',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(eventConfig.apiEndpoint || '/api/rsvp', {
                method: 'POST',
                headers: {
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800 rounded-2xl p-4 sm:p-5 space-y-3 flex flex-col justify-between">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white font-semibold text-base sm:text-lg">{eventConfig.speakerName}</p>
                                        <p className="text-xs text-zinc-400 mt-1">{eventConfig.speakerRole}<br/>{eventConfig.speakerTitle}</p>
                                    </div>
                                </div>
                                {eventConfig.speakerLinkedin && (
                                    <a 
                                        href={eventConfig.speakerLinkedin} 
                                        target="_blank" 
                            ...
                            <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800 rounded-2xl p-4 sm:p-5 space-y-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-white font-semibold text-base sm:text-lg">{eventConfig.eventDate}</p>
                                    <p className="text-xs text-zinc-400 mt-1">{eventConfig.eventTime}</p>
                                </div>
                            </div>
                        </div>
                    </div>
